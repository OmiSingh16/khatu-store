  ...
  useEffect(() => {
    ...
    const checkSeller = async () => {
      try {
        ...
        if (res.ok && data.isSeller) {
          setIsSeller(true);
          setStoreSlug(data.storeInfo?.slug || null);
        }
      } catch (err) {
        console.error("Footer seller check error:", err);
      }
    };
  ...
  const linkSections = [
    {
      title: "PRODUCTS",
      links: [
        { text: "Cooking Essentials", path: "/" },
        { text: "Spices", path: "/" },
        { text: "Dry Fruits", path: "/" },
        { text: "Household Items", path: "/" },
      ],
    },
    {
      title: "WEBSITE?",
      links: [
        ...
        // 🔥 SMART SELLER LINK
        {
          text: isSeller ? "Go to Your Store" : "Create Your Store",
          path: 
             "/create-store",
        },
      ],
    },
    {
      title: "CONTACT",
      links: [
        { text: "+91- 99919540573", path: "/", icon: MailIcon },
        { text: "shayamji@gmail.com", path: "/", icon: PhoneIcon },
        { text: "Mehndawal, Sant Kabir Nagar", path: "/", icon: MapPinIcon },
      ],
    },
  ];
        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
          <div>
            <Link href="/" className="text-4xl font-semibold text-slate-700">
              <span className="text-green-600">Khatu</span>Ji
              <span className="text-green-600 text-5xl leading-0">.</span>
            </Link>

            <p className="max-w-[410px] mt-6 text-sm">
              Welcome to Khatu-Shayam-Provision-Store, your neighborhood store for
              fresh groceries and daily essentials — now easier to shop online.
            </p>
